...
int nbEmployeParVille(Personne personne[], int NBRE_EMPLOYES, int codePostal);
...
int main(void)
{
    ...
    // TRAITEMENTS
    cout << "il y a "<< nbEmployeParVille(listeEmployes, NBRE_EMPLOYES, 64100) << " employes dans la ville de bayonne ";
...
int nbEmployeParVille(Personne personne[], int NBRE_EMPLOYES,  int codePostal)
{
    ...
    {
        if(personne[i].adresse.codePostal == codePostal)
        {
            nbEmploye += 1;
        }
    }
